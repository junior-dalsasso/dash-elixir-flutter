defmodule DashElixirFlutter.Serial do
  ...
  def init(:ok) do
    ...
    :ok = Circuits.UART.open(pid, "/dev/ttyUSB0", speed: 115_200, active: true)
  ...
  def handle_info(:tick, state) do
    ...
    :ok = Circuits.UART.write(state.uart, <<?A>>)
    ...
    case parse_data(data) do
      {:ok, parsed_data} ->
        if Process.whereis(DashElixirFlutter.StreamServer) do
          send(DashElixirFlutter.StreamServer, {:ecu_data, parsed_data})
        end
        {:noreply, state}
  ...
  @impl true
  def handle_call(:get_last_data, _from, %{last_data: last_data} = state) do
    {:reply, last_data, state}
  end
